...
async function buildPackage(name: string, file: string): Promise<void> {
  ...
  const output = await esbuild.build({
    ...
    alias: {
      ...
      assert: requireResolved('./mocks/assert.js'),
      path: requireResolved('./mocks/path.js'),
      typescript: requireResolved('./mocks/typescript.js'),
      'lru-cache': requireResolved('./mocks/lru-cache.js'),
    },
    plugins: [
      {
        name: 'replace-plugin',
        setup(build): void {
          build.onLoad(
            ...
            async args => {
              ...
              const text = await requireMock('./mocks/empty.js');
          ...
          build.onLoad(
            ...
            async args => {
              ...
              let text = await requireMock('./mocks/eslint-rules.js');
          ...
          build.onLoad(makeFilter('/eslint/lib/api.js'), async args => {
            ...
            let text = await requireMock('./mocks/eslint.js');
            // this is needed to bypass system module resolver
          ...
          build.onLoad(
            ...
            async args => {
              ...
              const text = await requireMock('./mocks/ts-estree.js');
              return { contents: text, loader: 'js' };
            },
          );
        },
